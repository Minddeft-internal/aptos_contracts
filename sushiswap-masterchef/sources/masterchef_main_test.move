module sushi_masterchef::masterchef_main_test {
    use std::string;
    use std::signer;
    use std::vector;
    ...
    const ERROR_INVALID_SUSHI_RATE: u64 = 5;
    const ERROR_PID_NOT_EXIST: u64 = 6;
    const ERROR_COIN_NOT_REGISTERED: u64 = 7;
    const ERROR_SUSHI_REWARD_OVERFLOW: u64 = 8;
    ...
    const RESOURCE_ACCOUNT: address = @sushi_masterchef;
    const APTOS_DEFAULT_DECIMAL: u8 = 8;
    const TOTAL_SUSHI_RATE_PRECISION: u64 = 100000;
    const INITIAL_REGULAR_SUSHI_RATE_PRECISION: u64 = 40000;
    const INITIAL_SPECIAL_SUSHI_RATE_PRECISION: u64 = 60000;
    const ACC_SUSHI_PRECISION: u128 = 1000000000000;
    ...
    struct MasterChef has key {
        ...
        sushi_per_second: u64,
        sushi_rate_to_regular: u64,
        sushi_rate_to_special: u64,
        last_upkeep_timestamp: u64,
        end_timestamp: u64
    }

    /*
    * Aptos coin can't transfer more than MAX_U64,when calculating sushi reward, in order to ensure the
    * sushi reward will not loss precision, many field will be multiplied like 'ACC_SUSHI_PRECISION'.
    * To make sure this will not cause overflow problem, we set 'user.amount','reward_debt','acc_sushi_pershare'
    * as u128,but when calculating the final sushi reward,it still will be cast to u64
    */
    ...
    struct UserInfo has store {
        amount: u128,
        ///   reward_debt is a 'accounting' field used for distribute sushi reward to each user in the pool.
        ///
        ///   pending_sushi_reward = (user.share * pool.acc_sushi_per_share) - user.reward_debt
        ///
        ///   Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        ///   1. 'update_pool': The pool info will be updated, that means 'acc_sushi_per_share' and 'last_reward_timestamp' updated
        ///   2. 'settle_pending_sushi': Masterchef will send all pending SUSHI to the user address.
        ///   3. 'reset reward_debt': user.reward_debt = user.share * pool.acc_sushi_per_share
        ///   it means that all the SUSHI rewards of the user have been taken away at this moment.
        ///
        reward_debt: u128
    }
    ...
    struct Events has key {
        ...
        update_sushi_rate_events: event::EventHandle<UpdateCakeRateEvent>,
    ...
    /// Event emitted when any action to farm pool will update acc_sushi_per_share info.
    ...
    /// Event emitted when update sushi emission rate.
    ...
    fun init_module(sender: &signer) {
        ...
        move_to(sender, MasterChef {
            ...
            sushi_per_second: 0,
            sushi_rate_to_regular: INITIAL_REGULAR_SUSHI_RATE_PRECISION,
            sushi_rate_to_special: INITIAL_SPECIAL_SUSHI_RATE_PRECISION,
        ...
        move_to(sender, Events {
            ...
            update_sushi_rate_events: account::new_event_handle<UpdateCakeRateEvent>(sender),
    ...
    public entry fun deposit<CoinType>(
        ...
        // Auto reigster sushi
        if (!coin::is_account_registered<Sushi>(sender_addr)) {
            coin::register<Sushi>(sender);
        };
        ...
        if (user_info.amount > 0) {
            let pending = (user_info.amount * pool_info.acc_sushi_per_share) / ACC_SUSHI_PRECISION - user_info.reward_debt;
            assert!(pending <= MAX_U64, ERROR_SUSHI_REWARD_OVERFLOW);
            let resource_signer = account::create_signer_with_capability(&master_chef_mut.signer_cap);
            safe_transfer_sushi(&resource_signer, sender_addr, (pending as u64))
        };
        if (amount > 0) {
        ...
        user_info.reward_debt = user_info.amount * pool_info.acc_sushi_per_share / ACC_SUSHI_PRECISION;
    ...
    public entry fun update_pool(pid: u64) acquires MasterChef, Events {
        let (sushi_reward, acc_sushi_per_share) = calc_sushi_reward(pid);
        let master_chef_mut = borrow_global_mut<MasterChef>(RESOURCE_ACCOUNT);
        let pool_info = vector::borrow_mut<PoolInfo>(&mut master_chef_mut.pool_info, pid);
        let current_timestamp = timestamp::now_seconds();

        if (sushi_reward > 0) {
            pool_info.acc_sushi_per_share = acc_sushi_per_share;
        };

        if (current_timestamp > pool_info.last_reward_timestamp) {
            // Timestamp will always be updated no matter sushi_reward is 0 or not
            ...
            event::emit_event<UpdatePoolEvent>(
                ...
                UpdatePoolEvent {
                    ...
                    acc_sushi_per_share: pool_info.acc_sushi_per_share
                }
            );
        };
    }

    public entry fun update_sushi_rate(
        sender: &signer,
        regular_farm_rate: u64,
        special_farm_rate: u64,
        with_update: bool
        ...
        assert!(
            regular_farm_rate + special_farm_rate == TOTAL_SUSHI_RATE_PRECISION,
            ERROR_INVALID_SUSHI_RATE
        );
        assert!(signer::address_of(sender) == borrow_global<MasterChef>(RESOURCE_ACCOUNT).admin, ERROR_NOT_ADMIN);
        ...
        let master_chef_mut = borrow_global_mut<MasterChef>(RESOURCE_ACCOUNT);
        master_chef_mut.sushi_rate_to_regular = regular_farm_rate;
        master_chef_mut.sushi_rate_to_special = special_farm_rate;

        let events = borrow_global_mut<Events>(RESOURCE_ACCOUNT);
        event::emit_event<UpdateCakeRateEvent>(
            &mut events.update_sushi_rate_events,
            UpdateCakeRateEvent {
                regular_farm_rate,
                special_farm_rate
            }
    ...
    /// On BNB chain, SUSHI is minted by the Masterchef contract at a speed of 40 SUSHIs
    /// per block, while on the Aptos chain, the mint of SUSHI is completely dependent on
    /// the bridged SUSHI numbers from the chain bridge, the caller should be the executor
    /// of the SUSHI bridge, automatically and regularly executes transactions, sends the
    /// SUSHI tokens to current contract, and then Masterchef distributes it to all pools user.
    ...
    public entry fun upkeep(
        ...
        if (!coin::is_account_registered<Sushi>(RESOURCE_ACCOUNT)) {
            coin::register<Sushi>(&account::create_signer_with_capability(&master_chef.signer_cap));
        };

        coin::transfer<Sushi>(sender, RESOURCE_ACCOUNT, amount);

        let current_timestamp = timestamp::now_seconds();
        let new_end_timestamp;
        let new_available_sushi = amount;

        if (current_timestamp <= master_chef.end_timestamp) {
            new_end_timestamp = master_chef.end_timestamp + elapsed;
            let remaining_sushi = (master_chef.end_timestamp - current_timestamp) * master_chef.sushi_per_second;
            new_available_sushi = new_available_sushi + remaining_sushi;
        } else {
            new_end_timestamp = current_timestamp + elapsed;
        };

        let new_sushi_per_second = new_available_sushi / (new_end_timestamp - current_timestamp);
        ...
        let master_chef_mut = borrow_global_mut<MasterChef>(RESOURCE_ACCOUNT);
        let prev_sushi_per_second = master_chef_mut.sushi_per_second;
        master_chef_mut.sushi_per_second = new_sushi_per_second;
        ...
        event::emit_event<UpkeepEvent>(
            ...
            UpkeepEvent {
                amount,
                elapsed,
                prev_sushi_per_second,
                sushi_per_second: master_chef_mut.sushi_per_second,
            }
        );
    }
    ...
    public fun pending_sushi(
        pid: u64,
        user: address
    ): u64 acquires MasterChef, PoolUserInfo {
        let (_, acc_sushi_per_share) = calc_sushi_reward(pid);

        assert!(exists<PoolUserInfo>(user), ERROR_POOL_USER_INFO_NOT_EXIST);
        let pool_user_info = borrow_global_mut<PoolUserInfo>(user);
        let user_info = Table::borrow_mut<u64, UserInfo>(&mut pool_user_info.pid_to_user_info, pid);

        ((user_info.amount * acc_sushi_per_share / ACC_SUSHI_PRECISION - user_info.reward_debt) as u64)
    }

    fun calc_sushi_reward(pid: u64): (u64, u128) acquires MasterChef {
        let master_chef = borrow_global<MasterChef>(RESOURCE_ACCOUNT);
	    let pool_info = vector::borrow<PoolInfo>(&master_chef.pool_info, pid);

        let sushi_reward:u128 = 0;
        let acc_sushi_per_share = pool_info.acc_sushi_per_share;
        let current_timestamp = timestamp::now_seconds();
        ...
        if (current_timestamp > pool_info.last_reward_timestamp) {
            ...
            if (pool_info.is_regular) {
                ...
                sushi_rate = master_chef.sushi_rate_to_regular;
            } else {
                total_alloc_point = master_chef.total_special_alloc_point;
                sushi_rate = master_chef.sushi_rate_to_special;
            };
            ...
            if (supply > 0 && total_alloc_point > 0) {
                // Aptos built-in maximum integer type is u128, when calculating sushi-reward
                // We must be very careful to the priority of multiply or division
                // As ususal,
                //
                // sushi_reward = (multiplier * master_chef.sushi_per_second * sushi_rate * pool_info.alloc_point) / (total_alloc_point * TOTAL_SUSHI_RATE_PRECISION)
                //
                ...
                sushi_reward = ((multiplier as u128) * (((master_chef.sushi_per_second as u128) * (sushi_rate as u128) * (pool_info.alloc_point as u128)) / (total_alloc_point as u128))) / (TOTAL_SUSHI_RATE_PRECISION as u128);
                acc_sushi_per_share = (pool_info.acc_sushi_per_share) + (sushi_reward * ACC_SUSHI_PRECISION) / supply;

                assert!(sushi_reward <= MAX_U64 && acc_sushi_per_share <= MAX_U128, ERROR_SUSHI_REWARD_OVERFLOW);
            };
        };

        ((sushi_reward as u64), acc_sushi_per_share)
    }

    fun safe_transfer_sushi(
        resource_signer: &signer,
        to: address, amount: u64
    ) {
        if (amount > 0) {
            let balance = coin::balance<Sushi>(RESOURCE_ACCOUNT);
            if (balance < amount) {
                amount = balance;
            };
            coin::transfer<Sushi>(resource_signer, to, amount);
        }
    }
    ...
    #[test_only]
    public fun create_sushilp_token()acquires MasterChef {
        let master_chef_mut = borrow_global_mut<MasterChef>(RESOURCE_ACCOUNT);
        let resource_signer = account::create_signer_with_capability(&master_chef_mut.signer_cap);

        managed_coin::initialize<Sushi>(
            &resource_signer,
            b"Sushi-Lp",
            b"SUSHI-LP",
            8,
     ...
     #[test_only]
    public fun register_and_mint(to: &signer, amount: u64)acquires MasterChef {

        let master_chef_mut = borrow_global_mut<MasterChef>(RESOURCE_ACCOUNT);
        let resource_signer = account::create_signer_with_capability(&master_chef_mut.signer_cap);    
    
        managed_coin::register<Sushi>(to);
        managed_coin::mint<Sushi>(&resource_signer, signer::address_of(to), amount)
    }
    ...
    public fun register(to: &signer) {
     ...
     #[test_only]
    public fun lp_balance(to:address):u64 {
        coin::balance<Sushi>(to)
    }

    #[test_only]
    public fun get_metadata_info(): (address, u64, u64, u64, u64, u64) acquires MasterChef {
        let master_chef = borrow_global<MasterChef>(RESOURCE_ACCOUNT);
        (master_chef.admin, master_chef.total_regular_alloc_point, master_chef.total_special_alloc_point, master_chef.sushi_per_second, master_chef.sushi_rate_to_regular, master_chef.sushi_rate_to_special)
    }
    ...
    public fun get_pool_info(pid: u64): (u128, u128, u64, u64, bool) acquires MasterChef {
        ...
        (pool_info.total_amount, pool_info.acc_sushi_per_share, pool_info.last_reward_timestamp, pool_info.alloc_point, pool_info.is_regular)
    }
