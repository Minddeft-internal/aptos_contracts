module sushi_multisig_wallet::multisig_wallet {
    use std::signer;
    use std::vector;
    use std::string;
    ...
    fun init_module(sender: &signer) {
        ...
        let _signer_cap = resource_account::retrieve_resource_account_cap(sender, @sushi_multisig_wallet_dev);
    }
    ...
    acquires MultisigWallet, MultisigTxs, MultisigWalletEvents {
        ...
        register_multisig_txs<WithdrawParams<TestSUSHI>>(&multisig_wallet);
        init_withdraw<TestSUSHI>(&sender, &multisig_wallet, eta, expiration, sender_addr, amount);
    }
    ...
    acquires MultisigWallet, MultisigTxs, MultisigWalletEvents {
        ...
        let multisig_txs = borrow_global<MultisigTxs<WithdrawParams<TestSUSHI>>>(multisig_wallet_addr);
        let tx = Table::borrow(&multisig_txs.txs, seq_number);
    ...
    acquires MultisigWallet, MultisigTxs, MultisigWalletEvents {
        ...
        coin::register<TestSUSHI>(&multisig_wallet);
        managed_coin::mint<TestSUSHI>(&sushi_multisig_wallet, signer::address_of(&multisig_wallet), 1000 * 100000000);
        ...
        let seq_number = next_seq_number(multisig_wallet_addr);
        register_multisig_txs<WithdrawParams<TestSUSHI>>(&multisig_wallet);
        init_withdraw<TestSUSHI>(&initiator, &multisig_wallet, eta, expiration, sender_addr, amount);
        approve_withdraw<TestSUSHI>(&approver, &multisig_wallet, seq_number);
        timestamp::fast_forward_seconds(HOUR_IN_SECONDS);
        coin::register<TestSUSHI>(&sender);
        execute_withdraw<TestSUSHI>(&sender, &multisig_wallet, seq_number);

        let multisig_txs = borrow_global<MultisigTxs<WithdrawParams<TestSUSHI>>>(multisig_wallet_addr);
        let tx = Table::borrow(&multisig_txs.txs, seq_number);
        assert!(tx.is_executed, 0);

        assert!(coin::balance<TestSUSHI>(sender_addr) == amount, 0);
    }
    ...
    fun before_each_test(
        ...
        init_module(sushi_multisig_wallet);

        resource_account::create_resource_account(sushi_multisig_wallet_dev, b"multisig_wallet", x"");
