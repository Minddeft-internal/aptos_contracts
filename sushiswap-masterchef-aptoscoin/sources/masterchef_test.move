...
module sushi_masterchef::masterchef_test {
    ...
    use sushi_masterchef::masterchef::{Self};
    ...
    fun test_all_in_one_upkeep(dev: &signer, admin: &signer, upkeep_admin: &signer, resource_account: &signer, user1: &signer, user2: &signer, user3: &signer, user4: &signer) {
        ...
        // register aptos lp token in user account
        masterchef::register(user4);

        // mint SUSHI for upkeep admin
        masterchef::register_and_mint(upkeep_admin,100000 * BASE_APTOS);
        ...
        masterchef::add_pool<TestSUSHI>(admin, 1, true, true);
        fast_forward_seconds(1);
        ...
        masterchef::deposit<TestSUSHI>(user1, 1 * BASE_APTOS);
        fast_forward_seconds(1);
        ...
        masterchef::deposit<TestSUSHI>(user2, 2 * BASE_APTOS);
        fast_forward_seconds(1);
        ...
        masterchef::withdraw<TestSUSHI>(user1, 1 * BASE_APTOS);
        fast_forward_seconds(1);
        ...
        masterchef::deposit<TestSUSHI>(user1, 2 * BASE_APTOS);
        fast_forward_seconds(1);
        ...
        masterchef::withdraw<TestSUSHI>(user1, 2 * BASE_APTOS);
        fast_forward_seconds(1);

        // W user2 withdraw 2 from pool 0
        masterchef::withdraw<TestSUSHI>(user2, 2 * BASE_APTOS);
        fast_forward_seconds(1);
        ...
        masterchef::deposit<TestSUSHI>(user2, 1 * BASE_APTOS);
        fast_forward_seconds(1);
        ...
        // AC admin update aptos rate to 30% 70%
        masterchef::update_aptos_rate(admin, 30000, 70000, true);
        fast_forward_seconds(1);
        ...
        let user1_balance_plus_pending = (
            masterchef::pending_aptos(0, signer::address_of(user1)) +
            masterchef::pending_aptos(1, signer::address_of(user1)) +
            masterchef::lp_balance(signer::address_of(user1))
        );
        let user2_balance_plus_pending = (
            masterchef::pending_aptos(0, signer::address_of(user2)) +
            masterchef::pending_aptos(1, signer::address_of(user2)) +
            masterchef::lp_balance(signer::address_of(user2))
        );
        let user3_balance_plus_pending = (
            masterchef::pending_aptos(2, signer::address_of(user3)) +
            masterchef::lp_balance(signer::address_of(user3))
        );
        let user4_balance_plus_pending = (
            masterchef::pending_aptos(3, signer::address_of(user4)) +
            masterchef::lp_balance(signer::address_of(user4))
        );
    ...
    fun test_all_in_one_delayed_upkeep(dev: &signer, admin: &signer, upkeep_admin: &signer, resource_account: &signer, user1: &signer, user2: &signer, user3: &signer, user4: &signer) {
        ...
        // mint APTOS for upkeep admin
         masterchef::register_and_mint(upkeep_admin,100000 * BASE_APTOS);


        // C admin upkeep elapsed 30s with 120 APTOS
        masterchef::upkeep(upkeep_admin, 120 * BASE_APTOS, 30, true);
        fast_forward_seconds(1);
        ...
        // AJ admin upkeep elapsed 30s with 90 APTOS
        masterchef::upkeep(upkeep_admin, 90 * BASE_APTOS, 30, true);
        fast_forward_seconds(1);
    ...
    //     // F user1 deposit 1 to pool 0
    //     masterchef::deposit<TestSUSHI>(user1, 0, 1 * BASE_APTOS);
    //     fast_forward_seconds(1);
    ...
    //     // H user2 deposit 2 to pool 0
    //     masterchef::deposit<TestSUSHI>(user2, 0, 2 * BASE_APTOS);
    //     fast_forward_seconds(1);
    ...
    //     // Q user1 withdraw 1 from pool 0
    //     masterchef::withdraw<TestSUSHI>(user1, 0, 1 * BASE_APTOS);
    //     fast_forward_seconds(1);
    ...
    //     // T user1 stake 2 in pool 0
    //     masterchef::deposit<TestSUSHI>(user1, 0, 2 * BASE_APTOS);
    //     fast_forward_seconds(1);
    ...
    //     // V user1 withdraw 2 from pool 0
    //     masterchef::withdraw<TestSUSHI>(user1, 0, 2 * BASE_APTOS);
    //     fast_forward_seconds(1);

    //     // W user2 withdraw 2 from pool 0
    //     masterchef::withdraw<TestSUSHI>(user2, 0, 2 * BASE_APTOS);
    //     fast_forward_seconds(1);
    ...
    //     // Z user2 stake 1 in pool 0
    //     masterchef::deposit<TestSUSHI>(user2, 0, 1 * BASE_APTOS);
    //     fast_forward_seconds(1);
    ...
    //     // AE admin update aptos per second to 2
    //     masterchef::update_aptos_per_second(admin, 2 * BASE_APTOS, true);
    //     fast_forward_seconds(1);
    ...
    //     // AN admin update aptos per second to 4
    //     masterchef::update_aptos_per_second(admin, 4 * BASE_APTOS, true);
    //     fast_forward_seconds(1);
    ...
    fun test_deposit(dev: &signer, admin: &signer, resource_account: &signer, user1: &signer, user2: &signer) {
        ...
        masterchef::deposit<TestSUSHI>(user1, 50);
        // one user deposit multiple times in same pool
        masterchef::deposit<TestSUSHI>(user1, 50);
        masterchef::deposit<TestSUSHI>(user2, 100);
        ...
        masterchef::deposit<TestSUSHI>(user1, 100);
    ...
    #[test(dev = @masterchef_origin, admin= @msterchef_admin, resource_account = @sushi_masterchef, user1 = @0x1234)]
    fun test_deposit_auto_register_aptos(dev: &signer, admin: &signer, resource_account: &signer, user1: &signer) {
        before_each(dev, admin, resource_account);
        before_add_pool<TestSUSHI>(resource_account, admin, b"SUSHI", b"SUSHI", 1000, true, true);
        account::create_account_for_test(signer::address_of(user1));
    ...
    fun test_withdraw(dev: &signer, admin: &signer, resource_account: &signer, user1: &signer, user2: &signer) {
        ...
        masterchef::deposit<TestSUSHI>(user1, 200);
        masterchef::deposit<TestSUSHI>(user2, 200);

        masterchef::withdraw<TestSUSHI>(user1, 50);
        masterchef::withdraw<TestSUSHI>(user1, 50);
        masterchef::withdraw<TestSUSHI>(user2, 100);
        ...
        masterchef::withdraw<TestSUSHI>(user1, 100);
        let (total_amount, _, _, _, _) = masterchef::get_pool_info(0);
    ...
    fun test_withdraw_insufficient_amount(dev: &signer, admin: &signer, resource_account: &signer, user1: &signer) {
        ...
        let (amount, _) = masterchef::get_user_info(0, signer::address_of(user1));
        assert!(amount == 0, DEFAULT_ERROR_CODE);
        masterchef::withdraw<TestSUSHI>(user1, 1);
    }
    ...
    fun test_emergency_withdraw(dev: &signer, admin: &signer, resource_account: &signer, user1: &signer, user2: &signer) {
        ...
        masterchef::emergency_withdraw<TestSUSHI>(user1);
        masterchef::withdraw<TestSUSHI>(user2, 100);
        ...
        masterchef::emergency_withdraw<TestSUSHI>(user2);
        let (total_amount, _, _, _, _) = masterchef::get_pool_info(0);
    ...
    fun test_add_pool_not_publish(dev: &signer, admin: &signer, resource_account: &signer) {
        before_each(dev, admin, resource_account);
        masterchef::add_pool<TestSUSHI>(admin, 1000, true, true);
    }
    ...
    fun test_add_pool_not_admin(dev: &signer, admin: &signer, resource_account: &signer) {
        ...
        masterchef::add_pool<TestSUSHI>(&random, 1000, true, true);
    }
    ...
    // #[test(dev = @masterchef_origin, admin= @msterchef_admin, resource_account = @sushi_masterchef)]
    // fun test_update_aptos_per_second(dev: &signer, admin: &signer, resource_account: &signer) {
    //     before_each(dev, admin, resource_account);
    //     before_add_pool<TestSUSHI>(resource_account, admin, b"SUSHI", b"SUSHI", 1000, false, true);

    //     masterchef::update_aptos_per_second(admin, 1000000000, true);

    //     let (_, _, _, aptos_per_second, _, _) = masterchef::get_metadata_info();
    //     assert!(aptos_per_second == 1000000000, DEFAULT_ERROR_CODE);
    // }

    // #[test(dev = @masterchef_origin, admin= @msterchef_admin, resource_account = @sushi_masterchef)]
    // #[expected_failure(abort_code = 0)]
    // fun test_update_aptos_per_second_not_admin(dev: &signer, admin: &signer, resource_account: &signer) {
    //     before_each(dev, admin, resource_account);
    //     // dev(not admin) updae aptos per second
    //     masterchef::update_aptos_per_second(dev, 1000000000, true);

    // }

    // #[test(dev = @masterchef_origin, admin= @msterchef_admin, resource_account = @sushi_masterchef)]
    // #[expected_failure(abort_code = 6)]
    // fun test_update_aptos_per_second_exceed_limit(dev: &signer, admin: &signer, resource_account: &signer) {
    //     before_each(dev, admin, resource_account);
    //     let max_per_second = 1000000000000;
    //     masterchef::update_aptos_per_second(admin, max_per_second + 1, true);
    // }

    #[test(dev = @masterchef_origin, admin= @msterchef_admin, resource_account = @sushi_masterchef)]
    fun test_update_aptos_rate(dev: &signer, admin: &signer, resource_account: &signer) {
        before_each(dev, admin, resource_account);
        before_add_pool<TestSUSHI>(resource_account, admin, b"SUSHI", b"SUSHI", 1000, false, true);

        let regular_rate = 60000;
        let special_rate = 40000;
        masterchef::update_aptos_rate(admin, regular_rate, special_rate, true);
    ...
    #[test(dev = @masterchef_origin, admin= @msterchef_admin, resource_account = @sushi_masterchef)]
    #[expected_failure(abort_code = 0)]
    fun test_update_aptos_rate_not_admin(dev: &signer, admin: &signer, resource_account: &signer) {
        before_each(dev, admin, resource_account);

        let regular_rate = 60000;
        let special_rate = 40000;
         // dev(not admin) set pool
        masterchef::update_aptos_rate(dev, regular_rate, special_rate, true);
    }

    #[test(dev = @masterchef_origin, admin= @msterchef_admin, resource_account = @sushi_masterchef)]
    #[expected_failure(abort_code = 5)]
    fun test_update_aptos_rate_invalid_rate(dev: &signer, admin: &signer, resource_account: &signer) {
        before_each(dev, admin, resource_account);

        let regular_rate = 60000;
        let special_rate = 40001;
        let total_aptos_rate = 100000;

        assert!(regular_rate + special_rate != total_aptos_rate, DEFAULT_ERROR_CODE);
        masterchef::update_aptos_rate(dev, regular_rate, special_rate, true);
    }
    ...
    public fun before_each(dev: &signer, admin: &signer, masterchef: &signer) {
        ...
        resource_account::create_resource_account(dev, b"sushi-swap-masterchef", x"2c967359c97aafbfc2fbfb56829303ae20434cb014bc9d1531fb9aba9cf1dd69");
        ...
        masterchef::create_aptos_token_for_test(aptos_framework);
    }
