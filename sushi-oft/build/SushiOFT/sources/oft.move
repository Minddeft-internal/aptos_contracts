module sushi_oft::oft {
    ...
    fun init_module(account: &signer) {
        let lz_cap = oft::init_oft<SushiOFT>(account, b"SushiSwap Token", b"Sushi", 8, 8);
        let signer_cap = resource_account::retrieve_resource_account_cap(account, @sushi_oft_origin);
    ...
    public entry fun lz_receive(src_chain_id: u64, src_address: vector<u8>, payload: vector<u8>) acquires OFT, OFTCap, Capabilities, EventStore {
        ...
        let oft = borrow_global<OFT>(@sushi_oft);
        ...
        oft::lz_receive<SushiOFT>(src_chain_id, src_address, payload)
    }
    ...
    public entry fun set_hard_cap(sender: &signer, chain_id: u64, cap: u64) acquires OFT, OFTCap {
        ...
        table::upsert(&mut borrow_global_mut<OFTCap>(@sushi_oft).hard_cap, chain_id, cap);
    }

    public entry fun whitelist(sender: &signer, addr: address, enable: bool) acquires OFT {
        assert_admin(signer::address_of(sender));
        let white_list = &mut borrow_global_mut<OFT>(@sushi_oft).white_list;
    ...
    public entry fun set_trust_remote(sender: &signer, chain_id: u64, remote_addr: vector<u8>) acquires OFT {
        assert_admin(signer::address_of(sender));
        let resource = account::create_signer_with_capability(&borrow_global<OFT>(@sushi_oft).signer_cap);
    ...
    public entry fun set_default_fee(sender: &signer, fee_bp: u64) acquires OFT {
        assert_admin(signer::address_of(sender));
        let resource = account::create_signer_with_capability(&borrow_global<OFT>(@sushi_oft).signer_cap);
        oft::set_default_fee<SushiOFT>(&resource, fee_bp)
    }
    ...
    fun non_blocking_lz_receive(src_chain_id: u64, src_address: vector<u8>, payload: vector<u8>, receiver: address, amount: u64) acquires Capabilities, EventStore {
        endpoint::lz_receive(src_chain_id, src_address, payload, &borrow_global<Capabilities>(@sushi_oft).lz_cap);
        let event_store = borrow_global_mut<EventStore>(@sushi_oft);
    ...
    fun assert_admin(account: address) acquires OFT {
        assert!(account == borrow_global_mut<OFT>(@sushi_oft).admin, ERROR_NOT_ADMIN);
    }
    ...
    fun test_send_and_receive_oft(
        ...
        test_helpers::deliver_packet<SushiOFT>(oracle, relayer, emitted_packet, 20);
